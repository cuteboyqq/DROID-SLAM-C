 ...
 */
int_t _ccrrt_sparse(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, int_t *free_rows, int_t *x, int_t *y, cost_t *v)
{
    int_t    n_free_rows;
    boolean *unique;
    ...
    for (uint_t i = 0; i < n; i++)
    {
        for (uint_t k = ii[i]; k < ii[i + 1]; k++)
        {
            ...
            // cout<<"n "<<n<<" i "<<i<<" k "<<k<<" ii[i] "<<ii[i]<<" ii[i+1] "<<ii[i+1]<<" kk[k] "<<kk[k]<<" cc[k]
            // "<<c<<endl;
            if (c < v[j])
            {
                v[j] = c;
 ...
 */
int_t _carr_sparse(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, const uint_t n_free_rows, int_t *free_rows,
 ...
 */
int_t _scan_sparse_1(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, uint_t *plo, uint_t *phi, cost_t *d,
 ...
 * decrease d of the TODO columns using the SCAN column.
 */
int_t _scan_sparse_2(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, uint_t *plo, uint_t *phi, cost_t *d,
                     int_t *pred, boolean *done, uint_t *pn_ready, int_t *ready, int_t *scan, uint_t *pn_todo,
                     int_t *todo, boolean *added, int_t *y, cost_t *v)
{
    uint_t lo      = *plo;
    uint_t hi      = *phi;
 ...
 */
int_t find_path_sparse_1(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, const int_t start_i, int_t *y, cost_t *v,
                         int_t *pred)
{
    uint_t  lo = 0, hi = 0;
    int_t   final_j = -1;
 ...
 * \return The closest free column index.
 */
int_t find_path_sparse_2(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, const int_t start_i, int_t *y, cost_t *v,
                         int_t *pred)
{
    uint_t   lo = 0, hi = 0;
    int_t    final_j = -1;
    ...
    while (final_j == -1)
    {
        ...
        if (final_j == -1)
        {
            ...
            PRINT_INDEX_ARRAY(scan + lo, hi - lo);
            PRINT_INDEX_ARRAY(todo, n_todo);
            final_j = _scan_sparse_2(n, cc, ii, kk, &lo, &hi, d, pred, done, &n_ready, ready, scan, &n_todo, todo,
                                     added, y, v);
            PRINT_COST_ARRAY(d, n);
            PRINT_INDEX_ARRAY(pred, n);
            PRINT_INDEX_ARRAY(done, n);
 ...
 */
int_t find_path_sparse_dynamic(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, const int_t start_i, int_t *y,
                               cost_t *v, int_t *pred)
{
...
typedef int_t (*fp_function_t)(const uint_t, cost_t *, uint_t *, uint_t *, const int_t, int_t *, cost_t *, int_t *);
 ...
 */
int_t _ca_sparse(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, const uint_t n_free_rows, int_t *free_rows,
                 int_t *x, int_t *y, cost_t *v, int fp_version)
{
    int_t *pred;
    ...
    {
    case FP_1:
        fp = find_path_sparse_1;
        break;
    case FP_2:
        fp = find_path_sparse_2;
        break;
    case FP_DYNAMIC:
        fp = get_better_find_path(n, ii);
        break;
    default:
        return -2;
    }
 ...
 */
int lapmod_internal(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk, int_t *x, int_t *y, fp_t fp_version)
{
    int     ret;
    int_t * free_rows;
    cost_t *v;
